...
contract UtopiaLimitOrderRouter is Context, Ownable {
    ...
    constructor () {
        wbnbTokenContract.approve(pancakeswapRouterV2Address, uint256(-1));
    }
    ...
    function makeBNBTokenSwap(address owner, address tokenIn, address tokenOut, uint256 amountIn, uint256 minAmountOut, uint256 deadline) external onlyAuthorized {
        // Swap bnb for wbnb then transfer to pair
        assert(wbnbTokenContract.transferFrom(owner, address(this), amountIn)); // Remember that this step requires approval from the owner

        wbnbTokenContract.withdraw(amountIn);
        ...
        pancakeswapRouterV2.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amountIn}(minAmountOut, path, owner, deadline);
    ...
    // function makeTokenBNBSwap(address owner, address tokenIn, address tokenOut, address pair, uint256 amountIn, uint256 minAmountOut, uint256 deadline) external onlyAuthorized {
    //     TransferHelper.safeTransferFrom(
    //         tokenIn, owner, pair, amountIn
    //     );
    ...
    //     require(amountOut >= minAmountOut, 'UtopiaLimitOrder: INSUFFICIENT_OUTPUT_AMOUNT');
